...
func resourceImageAssurancePolicyJson() *schema.Resource {
	return &schema.Resource{
		Create: resourceImageAssurancePolicyJsonCreate,
		Read:   resourceImageAssurancePolicyJsonRead,
		Update: resourceImageAssurancePolicyJsonUpdate,
		Delete: resourceImageAssurancePolicyJsonDelete,
...
func resourceImageAssurancePolicyJsonCreate(d *schema.ResourceData, m interface{}) error {
	ac := m.(*client.Client)
	...
	d.SetId(name)
	return resourceImageAssurancePolicyJsonRead(d, m)

}

func resourceImageAssurancePolicyJsonUpdate(d *schema.ResourceData, m interface{}) error {
	ac := m.(*client.Client)
	name := d.Get("name").(string)
	assurance_type := "image"
	...
	if d.HasChanges("description", "registry", "cvss_severity_enabled", "cvss_severity", "cvss_severity_exclude_no_fix", "custom_severity_enabled", "maximum_score_enabled", "maximum_score", "control_exclude_no_fix", "custom_checks_enabled",
		...
		if err == nil {
			err1 := resourceImageAssurancePolicyJsonRead(d, m)
			if err1 == nil {
				d.SetId(name)
			} else {
				return err1
...
func resourceImageAssurancePolicyJsonRead(d *schema.ResourceData, m interface{}) error {
	ac := m.(*client.Client)
	assurance_type := "image"
...
func resourceImageAssurancePolicyJsonDelete(d *schema.ResourceData, m interface{}) error {
	ac := m.(*client.Client)
	name := d.Get("name").(string)
	assurance_type := "image"
